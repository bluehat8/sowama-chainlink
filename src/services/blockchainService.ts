...
import { User, Activity, Environmental, TokenConfig } from "@/lib/types";
...
class BlockchainService {
  ...
  private tokenConfig: TokenConfig = {
    name: "SowamaToken",
    symbol: "SWT",
    initialValueUSD: 0.01, // Initial value: 1 cent per token
    currentValueUSD: 0.01, // Current value starts at initial value
    totalSupply: 1000000, // 1 million tokens initially
    circulatingSupply: 0,  // None circulating at launch
  ...
  // Update token value (admin only in a real application)
  async updateTokenValue(newValueUSD: number): Promise<boolean> {
    if (newValueUSD <= 0) return false;
    
    this.tokenConfig.currentValueUSD = newValueUSD;
    this.tokenConfig.lastUpdated = new Date().toISOString();
    
    console.log(`Token value updated to $${newValueUSD} USD`);
    return true;
  }
  
  // Calculate USD value of points
  async calculatePointsValueUSD(points: number): Promise<number> {
    return points * this.tokenConfig.currentValueUSD;
  }
  ...
  async addPoints(userId: string, points: number): Promise<boolean> {
    ...
    this.tokenConfig.circulatingSupply += points;
  ...
  async usePoints(userId: string, points: number): Promise<boolean> {
    ...
    // Update circulating supply
    this.tokenConfig.circulatingSupply = Math.max(0, this.tokenConfig.circulatingSupply - points);
